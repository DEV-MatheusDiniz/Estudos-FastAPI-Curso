...
async def get_curso(artigo_id: int, db: AsyncSession = Depends(get_session)):
    async with db as session:
        ...
        if artigo:
            ...
            raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail='Artigo não existe!!!')
